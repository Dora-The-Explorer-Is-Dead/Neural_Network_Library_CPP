...
public:
    ...
    void backward_with_timing(const Tensor& gradient) {
        // Backward through softmax (gradient already computed by loss function)
        Tensor grad = gradient;
        
        // Time each backward operation
        auto fc2_back_start = high_resolution_clock::now();
        grad = fc2.backward(grad);
        auto fc2_back_end = high_resolution_clock::now();

        auto dropout_back_start = high_resolution_clock::now();
        grad = dropout.backward(grad);
        auto dropout_back_end = high_resolution_clock::now();

        auto relu3_back_start = high_resolution_clock::now();
        grad = Activation::backward(bn3_out, grad, ActivationType::RELU);
        auto relu3_back_end = high_resolution_clock::now();

        auto bn3_back_start = high_resolution_clock::now();
        grad = bn3.backward(grad);
        auto bn3_back_end = high_resolution_clock::now();

        auto fc1_back_start = high_resolution_clock::now();
        grad = fc1.backward(grad);
        auto fc1_back_end = high_resolution_clock::now();

        auto flatten_back_start = high_resolution_clock::now();
        grad = flatten.backward(grad);
        auto flatten_back_end = high_resolution_clock::now();

        auto pool2_back_start = high_resolution_clock::now();
        grad = pool2.backward(grad);
        auto pool2_back_end = high_resolution_clock::now();

        auto relu2_back_start = high_resolution_clock::now();
        grad = Activation::backward(bn2_out, grad, ActivationType::RELU);
        auto relu2_back_end = high_resolution_clock::now();

        auto bn2_back_start = high_resolution_clock::now();
        grad = bn2.backward(grad);
        auto bn2_back_end = high_resolution_clock::now();

        auto conv2_back_start = high_resolution_clock::now();
        grad = conv2.backward(grad);
        auto conv2_back_end = high_resolution_clock::now();

        auto pool1_back_start = high_resolution_clock::now();
        grad = pool1.backward(grad);
        auto pool1_back_end = high_resolution_clock::now();

        auto relu1_back_start = high_resolution_clock::now();
        grad = Activation::backward(bn1_out, grad, ActivationType::RELU);
        auto relu1_back_end = high_resolution_clock::now();

        auto bn1_back_start = high_resolution_clock::now();
        grad = bn1.backward(grad);
        auto bn1_back_end = high_resolution_clock::now();

        auto conv1_back_start = high_resolution_clock::now();
        conv1.backward(grad);
        auto conv1_back_end = high_resolution_clock::now();

        // Print detailed timing
        cout << "\n=== DETAILED BACKWARD TIMING ===" << endl;
        cout << "FC2 back:   " << duration_cast<milliseconds>(fc2_back_end - fc2_back_start).count() << "ms" << endl;
        cout << "Dropout:    " << duration_cast<milliseconds>(dropout_back_end - dropout_back_start).count() << "ms" << endl;
        cout << "ReLU3:      " << duration_cast<milliseconds>(relu3_back_end - relu3_back_start).count() << "ms" << endl;
        cout << "BN3:        " << duration_cast<milliseconds>(bn3_back_end - bn3_back_start).count() << "ms" << endl;
        cout << "FC1 back:   " << duration_cast<milliseconds>(fc1_back_end - fc1_back_start).count() << "ms" << endl;
        cout << "Flatten:    " << duration_cast<milliseconds>(flatten_back_end - flatten_back_start).count() << "ms" << endl;
        cout << "Pool2:      " << duration_cast<milliseconds>(pool2_back_end - pool2_back_start).count() << "ms" << endl;
        cout << "ReLU2:      " << duration_cast<milliseconds>(relu2_back_end - relu2_back_start).count() << "ms" << endl;
        cout << "BN2:        " << duration_cast<milliseconds>(bn2_back_end - bn2_back_start).count() << "ms" << endl;
        cout << "Conv2:      " << duration_cast<milliseconds>(conv2_back_end - conv2_back_start).count() << "ms" << endl;
        cout << "Pool1:      " << duration_cast<milliseconds>(pool1_back_end - pool1_back_start).count() << "ms" << endl;
        cout << "ReLU1:      " << duration_cast<milliseconds>(relu1_back_end - relu1_back_start).count() << "ms" << endl;
        cout << "BN1:        " << duration_cast<milliseconds>(bn1_back_end - bn1_back_start).count() << "ms" << endl;
        cout << "Conv1:      " << duration_cast<milliseconds>(conv1_back_end - conv1_back_start).count() << "ms" << endl;
        cout << "================================\n" << endl;
    }
    ...
    try {
        ...
        for (int epoch = 0; epoch < EPOCHS; epoch++) {
            ...
            for (int i = 0; i < TRAIN_SAMPLES; i += BATCH_SIZE) {
                ...
                // Time backward pass with detailed breakdown
                ...
                if (num_batches == 1) {
                    network.backward_with_timing(loss_gradient);
                } else {
